...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("ammo_shotgun")
...
function ENT:PhysicsCollide(data, physobj)
	...
	if (data.Speed > 80 and data.DeltaTime > 0.2) then
		self.Entity:EmitSound("halo2/shotgun/shotgun_drop_soft"..math.random(1,3)..".wav")
	end
end
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) and not self.Planted then
		...
		self.Entity:EmitSound("halo2/shotgun/shotgun_ammo.wav")
		activator:GiveAmmo(12, "ammo_shotgun")
		self.Entity:Remove()
	end
end
...
function ENT:Draw()
	...
	local ledcolor = Color(0, 161, 255, 255)
  	...
  	local TargetPos = self.Entity:GetPos() + (self.Entity:GetUp() * 12.3) + (self.Entity:GetRight() * 2) + (self.Entity:GetForward() * 1.5)
